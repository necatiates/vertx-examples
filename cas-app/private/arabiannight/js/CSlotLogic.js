...
function _onSpin(iNumBettingLines, iCoin, iCurBet) {
    ...
    var bet = {
        lineCount: iNumBettingLines,
        bet: iCoin,
        minWin: 0,
        bonus: true,
        freeSpin: true,
        gameName: window.location.pathname.split("/")[2],
        curFreeSpinCnt: s_aSession["iTotFreeSpin"]
    };
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            s_aSession["bBonus"] = 0;
            betSuccess(iNumBettingLines, iCoin, iCurBet);
        },
        error: function () {
            alert('timeout');
        }
...
function betSuccess (iNumBettingLines, iCoin, iCurBet){
    ...
    if (_iCurRes._win_ || _iCurRes.bonus || _iCurRes.freeSpin) {
        ...
        if (s_aSession["bFreeSpin"] === 0 && s_aSession["bBonus"] === 0) {
            ...
            if (s_aSession["iTotFreeSpin"] === 0 && _iCurRes.freeSpin) {
                bFreespin = 1;

            } else if (_iCurRes.bonus) {
                bBonus = 1;
            }
        }
        ...
        if (bFreespin == 1 || bBonus == 1) {
            ...
            } while ((bFreespin != 1 && bBonus != 1) && (aRet.length === 0 || iTotWin > _iCurRes._maxWin_))
        }
        ...
        oData = "res=true&win=true&pattern=" + JSON.stringify(_aFinalSymbols) + "&win_lines=" + JSON.stringify(aRet) + "&money=" + s_aSession["iMoney"] +
        ...
        }while (checkWin(bFreespin, bBonus, iNumBettingLines).length != 0)
...
function _initSymbolWin() {
    _aSymbolWin[0] = [0, 0, 15, 300, 400];
    _aSymbolWin[1] = [0, 0, 40, 220, 320];
    _aSymbolWin[2] = [0, 0, 35, 200, 300];
    _aSymbolWin[3] = [0, 0, 25, 160, 220];
    _aSymbolWin[4] = [0, 0, 20, 120, 160];
    _aSymbolWin[5] = [0, 0, 15, 100, 140];
    _aSymbolWin[6] = [0, 0, 10, 15, 100];
    _aSymbolWin[7] = [0, 0, 0, 0, 0, 25];
    _aSymbolWin[8] = [0, 0, 0, 0, 0, 25];
    _aSymbolWin[9] = [0, 0, 0, 0, 0, 25];
...
function checkWin(bFreespin, bBonus, iNumBettingLines) {
    ...
    for (var k = 0; k < iNumBettingLines; k++) {
        ...
        aCellList.push({
            row: aCombos[0]['row'],
            col: aCombos[0]['col'],
            value: _aFinalSymbols[aCombos[0]['row']][aCombos[0]['col']]
        });
        ...
        while (iValue === 8 && iStartIndex < NUM_REELS) {
            ...
            aCellList.push({
                row: aCombos[iStartIndex]['row'],
                col: aCombos[iStartIndex]['col'],
        ...
        for (var t = iStartIndex; t < aCombos.length; t++) {
            if (_aFinalSymbols[aCombos[t]['row']][aCombos[t]['col']] === iValue ||
                ...
                aCellList.push({
                    row: aCombos[t]['row'],
                    col: aCombos[t]['col'],
                    value: _aFinalSymbols[aCombos[t]['row']][aCombos[t]['col']]
                });
            } else {
                break;
            }
        ...
        if (_aSymbolWin[iValue - 1][iNumEqualSymbol - 1] > 0) {
            _aWinningLine.push({
                line: k + 1,
                amount: _aSymbolWin[iValue - 1][iNumEqualSymbol - 1],
