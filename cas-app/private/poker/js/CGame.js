    ...
    this.dealCards = function(){
        ...
        if(_iCurRes._win_) {
            do {
                this._createCard();
                _bWinCurHand = true;
                calculatedWin = s_oPayTableSettings.getWin(_iCurCreditIndex, _oHandEvaluator.evaluate(_aCurHand)) * _iCurBet;
            } while (isNaN(calculatedWin) || calculatedWin == undefined || calculatedWin === 0 || calculatedWin > _iCurRes._maxWin_);
        }else{
            do {
                this._createCard();
                _bWinCurHand = false;
                calculatedWin = s_oPayTableSettings.getWin(_iCurCreditIndex, _oHandEvaluator.evaluate(_aCurHand)) * _iCurBet;
            } while (calculatedWin > 0);
        }
    ...
    this._onCardHide = function(){
        ...
        if(_bWinCurHand){
            do{
                ...
                var iRet = _oHandEvaluator.evaluate(_aCurHand);
                var calculatedWin = s_oPayTableSettings.getWin(_iCurCreditIndex,iRet) * _iCurBet;
            }while(isNaN(calculatedWin) || calculatedWin == undefined || calculatedWin == 0 || calculatedWin > _iCurRes._maxWin_);
        }else{
            do{
                s_oGame._changeCardValue();
