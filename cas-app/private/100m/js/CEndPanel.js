function CEndPanel(aInfos, iLevel){
    ...
    this._init = function(){
        ...
        _oScoreText = new createjs.Text(" RESULT " + s_oMain.getCurBet()," 22px "+FONT, "#ffffff");
        _oScoreText.x = 500;
        _oScoreText.y = 190;
    ...
    this.show = function(_iPlayerPositionArrive,_iCurRes){
        if(_iPlayerPositionArrive < 3){
            var winMultiplier = WIN_MULTIPLIER[_iPlayerPositionArrive];
            var totalWin = 0;
            var bet = {
                totalWin: winMultiplier * s_oMain.getCurBet(),
                id : _iCurRes.id
            };
            $.ajax({
                url: '/running/accept',
            ...
            _oScoreText.text = "Kazancınız : " + totalWin + " !";
        }else{
            _oScoreText.text = "Kaybettiniz";
        }
        var iX = 450;
        var iY = 210; 
