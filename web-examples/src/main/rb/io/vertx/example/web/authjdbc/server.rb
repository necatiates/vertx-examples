...
def set_up_initial_data(url)
  ...
  execute_statement("insert into user values ('tim', 'EC0D6302E35B7E792DF9DA4A5FE0DB3B90FCAB65A6215215771BF96D498A01DA8234769E1CE8269A105E9112F374FDAB2158E7DA58CDC1348A732351C38E12A0', 'C59EB438D1E24CACA2B1A48BC129348589D49303858E493FBE906A9158B7D5DC');")
...
router.route("/private/*").handler(&VertxWeb::RedirectAuthHandler.create(authProvider, "/index.html").method(:handle))
...
router.route("/loginhandler").handler(&VertxWeb::FormLoginHandler.create(authProvider).set_direct_logged_in_okurl("/private/games.html").method(:handle))
...
handler = VertxWeb::StaticHandler.create().set_caching_enabled(false).set_web_root("webroot")
router.route().handler() { |context|
  if (context.user() == nil)
    handler.handle(context)
  else
    context.response().put_header("location", "/private/games.html").set_status_code(302).end()
  end
}
