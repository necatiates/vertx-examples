...
function sizeHandler() {
	...
	$("#canvas").css("height",destH+"px");
        ...
        var iScratchMultiplier = destW / CANVAS_WIDTH;  
        
        destW = SCRATCHCANVAS_DIM * iScratchMultiplier;
	destH = SCRATCHCANVAS_DIM * iScratchMultiplier;
        
        $("#clear-image").css("width",destW+"px");
	$("#clear-image").css("height",destH+"px");
        
        if(fOffsetY < 0){
            $("#canvas").css("top",fOffsetY+"px");
            $("#clear-image").css("top", fOffsetY + (SCRATCH_Y*iScratchMultiplier )+"px");
        }else{
            $("#canvas").css("top","0px");
            $("#clear-image").css("top",  (SCRATCH_Y*iScratchMultiplier )+"px");
        }
        
        
        $("#canvas").css("left",fOffsetX+"px");
        $("#clear-image").css("left", fOffsetX + (SCRATCH_X*iScratchMultiplier )+"px");


};

	
//FORMAT FLOAT NUMBER WITH FIXED DECIMALS: (123456789.12345).formatMoney(2, '.', ',');
