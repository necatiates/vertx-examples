...
router.route("/private/*").handler(RedirectAuthHandler.create(authProvider, "/index.html").handle);

// Serve the static private pages from directory 'private'
router.route("/private/*").handler(StaticHandler.create().setCachingEnabled(false).setWebRoot("private").handle);

// Handles the actual login
router.route("/loginhandler").handler(FormLoginHandler.create(authProvider).setDirectLoggedInOKURL("/private/games.html").handle);
...
var handler = StaticHandler.create().setCachingEnabled(false).setWebRoot("webroot");
